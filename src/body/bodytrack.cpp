...
Pose* BodyTracker::GetPose(int userid) const
{
	...
	VX_ASSERT((userid >= 0) && (userid < MAX_USERS));
	pose = m_Users[userid].CurrentPose;
	if (pose == NULL)
	{
		pose = new Pose(skel);
		m_Users[userid].CurrentPose = pose;
	}
	pose->SetWorldPositions(m_Users[userid].Positions);
	pose->SetWorldRotations(m_Users[userid].Rotations);
	return pose;
}
...
bool BodyTracker::Eval(float t)
{
	if (m_UserID != 0)
	{
		...
		bool changed =	ComputeBoneAngles(m_UserID);
		...
		if (m_Options & TRACK_FRONT_ONLY)
		{
			...
			{
				VX_TRACE(Debug, ("BodyTracker user %d exit", m_UserID));
				OnPauseJointTrack(m_UserID);
				return true;
			}
			if (!m_Users[m_UserID].FacingSensor)
				return true;
		}
		m_Skeleton.Eval(t);
		m_PoseMapper->Eval(t);
...
/*!
 * @fn bool BodyTracker::ComputeBoneAngles(int userid)
 * @param userid	user ID of the user to drive the skeleton
 *
 ...
 *
 * @see KinectTracker::ComputeBonenalges
 */
bool BodyTracker::ComputeBoneAngles(int userid)
{
	return false;
}
...
/*!
 * @fn void BodyTracker::OnNewUser(int userid)
 ...
 * @see BodyTracker::OnStartJointTrack BodyTracker::OnStopBodyTrack
 */
void BodyTracker::OnNewUser(int userid)
{
	VX_TRACE(Debug, ("BodyTracker new user %d", userid));
	VX_ASSERT(userid < MAX_USERS);
	m_Users[userid].UserID = userid;
	m_Users[userid].FacingSensor = false;
	if (IsSet(SharedObj::DOEVENTS) && IsActive())
	{
		TrackEvent* ev = new TrackEvent(TrackEvent::NEW_USER, userid, this);
		ev->Log();
	}
}
...
/*!
 * @fn void BodyTracker::OnStartJointTrack(int userid)
 * @param userid	User ID of new user
 *
 ...
 */
void BodyTracker::OnStartJointTrack(int userid)
{
	Skeleton*	target = (Skeleton*) GetTarget();

	VX_ASSERT(userid < MAX_USERS);
	m_Users[userid].UserID = userid;
	m_Users[userid].Tracking = true;
	if (m_UserID == 0)
	{
		if (IsSet(SharedObj::DOEVENTS))
		{
			TrackEvent* ev = new TrackEvent(TrackEvent::START_BODY_TRACK, userid, this);
			ev->Log();
		}
		m_UserID = userid % MAX_USERS;
	}
	VX_TRACE(Debug, ("BodyTracker started tracking user %d", userid));
}
 ...
 * @see BodyTracker::OnNewUser BodyTracker::OnPauseJointTrack BodyTracker::OnStopJointTrack
 */
void BodyTracker::OnStopJointTrack(int userid)
{
	VX_ASSERT(userid < MAX_USERS);
	m_Users[userid].UserID = 0;
	m_Users[userid].FacingSensor = false;
	m_Users[userid].Tracking = false;
	if (m_UserID == userid)
	{
		if (IsSet(SharedObj::DOEVENTS))
		{
			TrackEvent* ev = new TrackEvent(TrackEvent::STOP_BODY_TRACK, userid, this);
			ev->Log();
		}
		m_UserID = 0;
	}
	VX_TRACE(Debug, ("BodyTracker stopped tracking user %d", userid));
}

/*!
 * @fn void BodyTracker::OnPauseJointTrack(int userid)
 * @param userid	User ID of user
 *
 * Callback invoked when the sensor temporarily stops tracking a user
 * (the leave the frame or cannot be tracked).
 * If DOEVENTS is enabled, a TrackEvent of type STOP_BODY_TRACK is logged.
 *
 * @see BodyTracker::OnNewUser BodyTracker::OnPauseJointTrack BodyTracker::OnStopJointTrack
 */
void BodyTracker::OnPauseJointTrack(int userid)
{
	if (m_UserID == userid)
	{
		if (IsSet(SharedObj::DOEVENTS))
		{
			TrackEvent* ev = new TrackEvent(TrackEvent::PAUSE_BODY_TRACK, userid, this);
			ev->Log();
		}
		m_UserID = 0;
	}
	VX_TRACE(Debug, ("BodyTracker paused tracking user %d", userid));
}
