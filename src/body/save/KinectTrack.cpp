 ...
 * Avoids a blocking wait for new frame data.
 */
class NiteListener : public nite::UserTracker::Listener
{
public:
	NiteListener(Kinect* kinect)
...
bool KinectTracker::Eval(float t)
{
	...
	{
		m_Kinect->Update();			// handled by NiteListener
...
/*!
 * @fn bool KinectTracker::ComputeBoneAngles(int userindex)
 ...
 */
bool KinectTracker::ComputeBoneAngles(int userindex)
{
	...
	bool		changed = false;
	Quat		(&rotations)[Skeleton::NUM_BONES] = m_Users[userindex].Rotations;
	Vec3		(&positions)[Skeleton::NUM_BONES] = m_Users[userindex].Positions;
	int			kinectID = m_Users[userindex].UserID;
	...
	for (int i = 0; i < Skeleton::NUM_BONES; ++i)
	{
		...
		switch (i)
		{
			...
			if (m_UserIndex == userindex)
				OnUserMove(userindex, i, wpos, rotations[i]);
			break;
	...
	wpos = m_WaistOffset;
			...
			m_UserPos = wpos;
			VX_TRACE(BodyTracker::Debug, ("KinectTracker: TORSO kinect(%f, %f, %f) world(%f, %f, %f)", kpos.x, kpos.y, kpos.z, wpos.x, wpos.y, wpos.z));
	...
	ApplyCurrentPose(userindex, rotchanged);
	return true;
}
...
//	UserTracker.addListener(&SkeletonListener);
	IsStarted = true;
	return true;
}
...
void NiteListener::onNewFrame(nite::UserTracker& ut)
{
	Owner->Update();
}
...
void Kinect::Update()
{
	...
	ObjectLock			lock(tracker);
	BodyTracker::User	(&vxusers)[BodyTracker::MAX_USERS](tracker->GetUsers());

	for (int i = 0; i < users.getSize(); ++i)
	{
		const nite::UserData&	user = users[i];
		nite::UserId			id = user.getId();
		int						j = tracker->FindUserIndex(id);
...
void Kinect::LostUser(nite::UserId userid)
{
	...
	Owner->OnLostUser(userid);
	UserTracker.stopSkeletonTracking(userid);
}
 ...
 */
void  Kinect::EnterUser(nite::UserId id)
{
