...
extern bool _cdecl CoreInit();
extern void _cdecl CoreExit();

#if 0
/*!
 * @class TLS
 * @param VAL	class for value of thread storage variable
 *
 * @brief Container for a thread local storage variable.
 *
 * Every time a member of this class is instantiated, it reserves space
 * in the thread local storage block. Thread local storage provides a
 * different memory area for each thread. Thread local variables reference
 * values in this area, allowing the variable to have different values
 * in different threads.
 *
 * This class is templatized so that the type of the variable can be
 * any class. Thread local storage is very limited so it is a good idea
 * to keep the number and size of TLS variables small.
 *
 * @ingroup vcore
 * @see TLSData
 */
template <class VAL> class TLS
{
public:
	//! Creates an uninitialized variable in thread local storage.
	TLS();
	
	//!	 Creates a variable in thread local storage with the given value.
	TLS(VAL v);
	
	//!	Converts a TLS variable into its value.
	operator VAL() const;
	
	//!	Updates the value of the TLS variable for the current thread.
	TLS&	operator=(VAL v);

	//! Returns a pointer to the thread local variable value for the current thread.
	VAL*	Get() const;
...
//! Returns first interface supported by this class.
	VInterface*		GetBaseInterface() 					{ return m_pBaseInterface; }
//! Returns next interface.
	VInterface*		GetNextInterface() 					{ return m_pNextInterface; }
...
//! Returns next COM interface
	VInterface*		NextComInterface ();
//! Returns COM class factory`
	VInterface*		ComClassFactory ();
...
//! Constructors choose what and how to iterate.
	VInterfaceIter (const BaseObj* pObj, Sequence = ITER_JustClassInterfaces);		// interfaces in pObj or base class
	VInterfaceIter (Class* pClass, Sequence = ITER_JustClassInterfaces);		// interfaces implemented in+below pClass
	VInterfaceIter (Sequence sequence);											// entire class hierarchy
