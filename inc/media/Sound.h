 ...
 */
class PSSoundListener : public PSEngine
{
public:
	PS_DECLARE_CLASS(PSSoundListener);
	PSSoundListener();
	~PSSoundListener();
	...
	VDebug&		Print(VDebug& = psm_debug) const;
...
private:
	...
	PSVec3						m_UpOrientation;		// Vector describing the up orientation of the listener
	PSVec3						m_ForwardOrientation;	// Vector describing the forward orientation of the listener
	PSVec3						m_Position;				// Vector describing the position of the listener
	PSVec3						m_Velocity;				// Vector describing the velocity of the listener
	...
	VString						m_FileName;				// The current midi file name
...
inline const char*	PSSoundListener::GetFileName() const
{	return m_FileName.IsEmpty() ? NULL : (const char*) m_FileName; }

inline LPDIRECTSOUND8	PSSoundListener::GetListener() const
{	return m_pDS; }
...
class PSSoundSource : public PSEngine
{
public:
	PS_DECLARE_CLASS(PSSoundSource);
	PSSoundSource();
	~PSSoundSource();
...
private:
	Ref<PSSoundListener>	m_Listener;				// to get to the primary sound buffer when needed
	...
	VString					m_FileName;				// name of the wave file
	PSVec3					m_Position;				// vector describing the position of the sound
	PSVec3					m_Velocity;				// vector describing the velocity of the sound
	PSVec3					m_ConeOrientation;		// vector describing the sound cone orientation
	...
	PSSoundData*			m_SoundData;			// holds the wave file data
	int						m_init;					// holds flags for when the DirectSound objects have been created
};


inline const char*	PSSoundSource::GetFileName() const
{	return m_FileName.IsEmpty() ? NULL : (const char*) m_FileName; }
